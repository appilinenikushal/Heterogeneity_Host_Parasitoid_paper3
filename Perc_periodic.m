function [Data] = Perc_periodic(L,l,g,eh,ep,t,p_location,p_density,h_indices,sites)
...
for i=1:t
    ...
    EH(1,2:L+1) = H(L,:); %periodic boundary, south to north
    EH(L+2,2:L+1) = H(1,:); %periodic boundary, north to south
    EH(2:L+1,1) = H(:,L); %%periodic boundary, west to east
    EH(2:L+1,L+2) = H(:,1); %periodic boundary, east to west

    EP(1,2:L+1) = P(L,:); %periodic boundary, south to north
    EP(L+2,2:L+1) = P(1,:); %periodic boundary, north to south
    EP(2:L+1,1) = P(:,L); %%periodic boundary, west to east
    EP(2:L+1,L+2) = P(:,1); %periodic boundary, east to west
